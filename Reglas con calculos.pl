...
distancia(X1,X2,Y1,Y2):- write("d=sqrt((x_2-x_1)^2+(y_2-y_1)^2)"), nl,
...
f_g(Ax, Bx, Ds) :- 	Ax =\= 0, Ds < 0, 
					Bx is Bx, 
					write("Su resultado, no se encuentra en los numeros reales").

f_g(Ax, Bx, Ds) :- 	Ax =\= 0, Ds >= 0, Dx is sqrt(Ds),
					X1 is ((-Bx + Dx) / (2 * Ax)), 
					X2 is ((-Bx - Dx) / (2 * Ax)), 
					write("X1 = "), write(X1), nl,
					write("X2 = "), write(X2).
				
f_g(Ax, Bx, Ds) :- Ax =:= 0, Bx is Bx, Ds is Ds, write("Su resultado es indeterminado.").
